...
app.put('/api/profile', authenticateToken, (req, res) => {
  const { valorant_id, rank, role, favorite_agents } = req.body;
  db.prepare('UPDATE users SET valorant_id = ?, rank = ?, role = ?, favorite_agents = ? WHERE id = ?')
    .run(valorant_id, rank, role, JSON.stringify(favorite_agents || []), req.user.id);
  res.json({ success: true });
});

app.get('/api/rooms', (req, res) => {
  const rooms = db.prepare(`SELECT r.*, u.username, u.valorant_id as owner_valorant_id, u.rank, u.role as owner_role, u.favorite_agents as owner_favorite_agents
    FROM rooms r JOIN users u ON r.user_id = u.id
    WHERE r.expires_at > datetime('now') ORDER BY r.created_at DESC`).all();
  const formattedRooms = rooms.map(room => {
    const participants = db.prepare(`SELECT rp.is_ready, u.id, u.username, u.valorant_id, u.rank, u.role, u.favorite_agents
...
app.post('/api/rooms', authenticateToken, (req, res) => {
  const { mode, age, description, agents, microphone, team_size } = req.body;
  const expires_at = new Date(Date.now() + 2 * 60 * 60 * 1000).toISOString();
  const stmt = db.prepare('INSERT INTO rooms (user_id, mode, age, description, agents, microphone, team_size, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
  const result = stmt.run(req.user.id, mode, age, description, JSON.stringify(agents || []), microphone ? 1 : 0, team_size || 5, expires_at);
...
app.post('/api/rooms/:id/ready', authenticateToken, (req, res) => {
...
app.get('/api/rooms/:id/messages', authenticateToken, (req, res) => {
  const messages = db.prepare(`SELECT rm.id, rm.message, rm.created_at, u.username as sender
    ...
    WHERE rm.room_id = ? ORDER BY rm.created_at ASC LIMIT 100`).all(req.params.id);
...
app.post('/api/matchmaking/join', authenticateToken, (req, res) => {
  ...
  const queue = db.prepare(`SELECT mq.user_id, u.username, u.rank, u.role, u.valorant_id, u.favorite_agents
    FROM matchmaking_queue mq JOIN users u ON mq.user_id = u.id ORDER BY mq.joined_at ASC`).all();

  if (queue.length >= 5) {
    const team = queue.slice(0, 5);
    const allAgents = team.flatMap(t => t.favorite_agents ? JSON.parse(t.favorite_agents) : []);
    const uniqueAgents = [...new Set(allAgents)];
    const hasDuplicate = uniqueAgents.length < allAgents.length;

    if (hasDuplicate) {
      db.prepare('DELETE FROM matchmaking_queue WHERE user_id = ?').run(req.user.id);
      return res.json({ matched: false, waiting: queue.length - 1, message: 'Ajan çakışması var, tekrar deneniyor...' });
    }
